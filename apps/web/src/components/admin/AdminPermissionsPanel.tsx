          ...
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              <tr key={row.id} className="border-t border-zinc-800/80 hover:bg-zinc-900/40">
                <td className="px-3 py-2.5 font-mono text-xs text-zinc-300">{row.slug}</td>
                <td className="px-3 py-2.5 text-zinc-400">{row.domain}</td>
                <td className="px-3 py-2.5">
                  <RiskBadge tier={row.risk_tier} />
                </td>
                <td className="px-3 py-2.5 text-zinc-400">{row.description ?? row.name}</td>
              </tr>
            ))
          )}
        </tbody>
