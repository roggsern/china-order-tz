...
function MegaMenuPanelStatus({
  ...
  if (isLoading) {
    return (
      <div className="space-y-3 p-6" data-testid="china-mega-loading" aria-busy="true">
  ...
  if (error) {
    return (
      <div className="space-y-3 p-6" data-testid="china-mega-error" role="alert">
        <p className="text-[13px] font-medium text-zinc-800">Unable to load China menu.</p>
  ...
  if (empty) {
    return (
      <div className="space-y-3 p-6" data-testid="china-mega-empty">
        <p className="text-[13px] text-zinc-600">Categories will appear once published.</p>
        <Link
          href="/products?origin=china"
          className="inline-flex items-center gap-1.5 text-[13px] font-semibold text-[#c9a227]"
        >
 ...
 * Trigger is always interactive; panel shows skeleton/error while /china/menu loads.
 */
export function MegaMenu({
  ...
  const { menu, isLoading, error } = useChinaStorefrontMenu(activeSlug || undefined, {
    ...
    return (
      <div className="max-h-[min(70vh,32rem)] overflow-y-auto [scrollbar-width:thin]">
        <button
          ...
          data-testid="china-mega-trigger"
        >
        ...
        {open ? (
          <div id={menuId} className="pb-1.5 pt-1">
            {statusPanel && ((isLoading && !menu) || error || categories.length === 0) ? (
              statusPanel
            ) : (
              <>
  ...
  return (
    ...
    >
      <button
        type="button"
        className={`${triggerClassName} ${open ? "bg-zinc-50/90 text-zinc-900" : ""}`}
        aria-expanded={open}
        aria-haspopup="true"
        aria-controls={menuId}
      ...
      >
        <div className="max-h-[calc(100vh-5.5rem)] overflow-hidden rounded-2xl border border-zinc-200/90 bg-white shadow-[0_20px_50px_-12px_rgba(0,0,0,0.18)] ring-1 ring-zinc-900/[0.04]">
          {(isLoading && !menu) || error || (!isLoading && categories.length === 0) ? (
            statusPanel
          ) : activeCategory ? (
                  ...
                  <div className="flex items-start justify-between gap-5">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1.5 max-w-md text-[13px] leading-relaxed text-zinc-500">
                        {presentation?.description || "Import products directly from China."}
                      </p>
                    </div>
                    <Link
