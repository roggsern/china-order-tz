...
type ProductSeed = Omit<
  Product,
  "featured" | "status" | "badges" | "trustBadges" | "origin" | "specifications" | "customerReviews"
>;
...
function enrichProduct(seed: ProductSeed, index: number): Product {
  const meta = productMetaBySlug[seed.slug] ?? {};
  const origin = meta.origin ?? "china";
  const badges = resolveProductBadges(seed.badge, seed.stock);
  const trustBadges = meta.trustBadges ?? getDefaultTrustBadges(origin, seed.rating);
  const specifications =
    meta.specifications ?? getDefaultSpecifications(seed.features, origin);
  const customerReviews =
    meta.customerReviews ?? getDefaultReviews(seed.name, seed.rating);
...
export const SEED_PRODUCTS: Product[] = productSeeds.map(enrichProduct);
