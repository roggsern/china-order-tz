/**
 * Mobile purchase-quantity presentation.
...
export function resolveQuotePurchaseQuantity(
  ...
  return quote.purchaseQuantity ?? null;
}

export function shouldBlockCheckoutCta(
  blockers: readonly PurchaseQuantityBlocker[] | null | undefined,
): boolean {
  return Array.isArray(blockers) && blockers.length > 0;
}

/** Disable start until the authenticated cart response is present. */
export function isCheckoutStartBlockedByPurchaseState(input: {
  cartLoaded: boolean;
  blockers: readonly PurchaseQuantityBlocker[] | null | undefined;
}): boolean {
  if (!input.cartLoaded) {
  ...
  return shouldBlockCheckoutCta(input.blockers);
}
...
export function resolvePdpPurchaseQuantityView(
  ...
  return {
    ...
    incrementLabel:
      presentation.increment != null
        ? `Order increment: ${presentation.increment}`
        : null,
...
export function resolveCartBlockerView(
  ...
  return {
    ...
    status,
    nextAllowed,
    mixVariants: aggregatesVariants
      ? 'Any variant counts toward this total.'
      : null,
  };
}
...
export function formatPurchaseQuantityCheckoutMessage(
  ...
  if (
    !blocker.increment_satisfied &&
    blocker.next_legal_quantity !== blocker.eligible_quantity
  ) {
...
export function purchaseQuantityMessageFromError(error: unknown): string | null {
  ...
  const payload = record.raw ?? error;
  const parsed = parsePurchaseQuantityCheckoutError(payload);
  const code =
    parsed.code ?? (typeof record.code === 'string' ? record.code : null);

  if (code === 'purchase_quantity_unsatisfied') {
    return (
  ...
  if (parsed.blocker) {
    return formatPurchaseQuantityCheckoutMessage(parsed.blocker);
  }
